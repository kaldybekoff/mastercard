...
| **PR-AUC** | Основная. Честнее ROC-AUC при дисбалансе классов |
| **ROC-AUC** | Дополнительная, для полноты |
| **Precision@K** | Бизнес-метрика: из top-K карт, какая доля = реально бизнес |
| **F1** | Дополнительная |
| **Recall (Synthetic Injection)** | Уникальная оценка recall через inject-тест |

**Почему PR-AUC, а не ROC-AUC как основная:**
При сильном дисбалансе (много true negatives) ROC-AUC выглядит хорошо даже у плохой модели. PR-AUC фокусируется на качестве работы с позитивным классом.
