...
"""
import polars as pl
from src.config import B2B_MCC_CODES, CONSUMER_MCC_CODES, MIXED_MCC_CODES, RENTAL_MCC_CODES

_B2B = [str(c) for c in B2B_MCC_CODES]
_CONSUMER = [str(c) for c in CONSUMER_MCC_CODES]
_MIXED = [str(c) for c in MIXED_MCC_CODES]
_RENTAL = [str(c) for c in RENTAL_MCC_CODES]
...
def compute_mcc_features(df: pl.DataFrame) -> pl.DataFrame:
    df = df.with_columns([
        ...
        pl.col("mcc").is_in(_RENTAL).alias("_is_rental"),
    ...
    agg = df.group_by("card_number").agg([
        ...
        pl.col("_is_rental").sum().alias("_rental_tx"),
        ...
        pl.when(pl.col("_is_rental")).then(pl.col("_amt")).otherwise(None).sum().alias("_rental_spend"),
    ...
    rental_mcc_uniq = (
        df.filter(pl.col("_is_rental"))
        .group_by("card_number")
        .agg(pl.col("mcc").n_unique().alias("n_unique_rental_mcc"))
    )

    result = (
        agg.join(totals, on="card_number")
        .join(b2b_mcc_uniq, on="card_number", how="left")
        .join(rental_mcc_uniq, on="card_number", how="left")
        .with_columns([
            pl.col("n_unique_b2b_mcc").fill_null(0),
            pl.col("n_unique_rental_mcc").fill_null(0),
            ...
            (pl.col("_rental_tx") / pl.col("_total_tx")).alias("rental_tx_share"),
            (pl.col("_rental_spend").fill_null(0) / pl.col("_total_spend")).alias("rental_spend_share"),
        ...
        .select([
            ...
            "mixed_tx_share", "mixed_spend_share",
